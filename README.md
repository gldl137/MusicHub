...
- **双插件体系**：
  - **MusicFree 插件**：100% 兼容 [MusicFree](https://github.com/maotoumao/MusicFree) 插件，热加载（放入目录即生效，无需重启）
  - **落雪（LX）自定义音源**：兼容落雪音乐的自定义音源脚本（`lx` API 沙箱，支持 kw / kg / tx / wy / mg 与 128k~flac24bit 音质），用于播放解析
- **双音源体系**：
...
插件体系同时支持 **MusicFree 插件** 与 **落雪（LX）自定义音源** 两类脚本，均热加载、无需重启，也可在 Web 端「插件管理」中从 URL 安装与启用/禁用：

| 类型 | 存放目录 | 作用 |
|---|---|---|
| MusicFree 插件 | `app/data/plugins/MF/` | 搜索 / 榜单 / 歌单 / 播放地址 / 歌词 / 封面 等完整音源能力 |
| 落雪自定义音源 | `app/data/plugins/LX/` | 播放解析（musicUrl），支持多音质，与落雪桌面端音源脚本通用 |

> MusicFree 插件提供"从哪找歌"的能力，落雪音源负责"怎么把歌播出来（取播放直链）"，两者可配合使用；只装其一也能正常工作。
